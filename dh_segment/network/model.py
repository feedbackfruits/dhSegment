...
from .pretrained_models import vgg_16_fn, resnet_v1_50_fn, resnet_v1_152_fn
...
def inference_resnet_v1_152(images, params, num_classes, use_batch_norm=False, weight_decay=0.0,
                           is_training=False) -> tf.Tensor:
    if use_batch_norm:
        if params.batch_renorm:
            renorm_clipping = {'rmax': 100, 'rmin': 0.1, 'dmax': 1}
    ...
    # Original ResNet
    blocks_needed = max([i for i, is_needed in enumerate(params.selected_levels_upscaling) if is_needed])
    resnet_net, intermediate_layers = resnet_v1_152_fn(images, is_training=False, blocks=blocks_needed,
                                                      weight_decay=weight_decay, renorm=False,
                                                      corrected_version=params.correct_resnet_version)
